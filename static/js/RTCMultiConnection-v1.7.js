...
//			       log('Message already received.');
				return setTimeout(repeatedlyCheck, 2000);
			}
//			log('NEW Message received.')
			...
			setTimeout(repeatedlyCheck, 2000);
	      });
	}


	function repeatedlyCheckDefault() {
		var practice_pk = document.getElementById('practice_pk').innerHTML;
		xhr('/practice/default_channel_check/'+practice_pk+'/', function (data) {
			//if server says nothing; wait.
			if (data == false) return setTimeout(repeatedlyCheckDefault, 2000);
			
			//if already receied same message; skip.
			if (messagesReceived[data[0].pk]){
//			       log('Message already received.');
				return setTimeout(repeatedlyCheckDefault, 2000);
			}
//			log('NEW Message received.')
			...
			// don't pass self messages over "onmessage" handlers
			if (session_data.sender != connection.userid && onDefaultMessageCallbacks[session_data.channel]) {
			      onDefaultMessageCallbacks[session_data.channel](session_data.message);
			}

			// repeatedly check the database
			setTimeout(repeatedlyCheckDefault, 2000);
	      });
	}
	    ...
	    connection.openDefaultSignalingChannel = function (config) {
              //  var channel = config.channel || this.channel;
                var channel = this.channel;
                log('channel variable is:'+channel);
                onDefaultMessageCallbacks[channel] = config.onmessage;
...
//                log(sdp.type, sdp.sdp);
        ...
        var defaultSocket = connection.openDefaultSignalingChannel({
